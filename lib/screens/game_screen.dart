...
class _GameScreenState extends State<GameScreen>
        ...
        return Stack(children: [
          SafeArea(
            child: Column(children: [
              ...
              Text('${st.path.length} / $total CELLS',
                  style: techno(11, color: kTextDim, letterSpacing: 2)),
              Expanded(child: Center(child: _board(st))),
              Text('DRAG FROM THE TEAL DOT · COVER EVERY CELL TO THE PINK DOT',
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          ...
          Text('RESTARTS', style: techno(8, color: kTextDim, letterSpacing: 1.5)),
        ]),
      ]),
    );
  }
  ...
  Widget _board(GameState st) {
    ...
    int? cellAt(Offset p) {
      ...
      if (r < 0 || c < 0 || r >= st.size || c >= st.size) return null;
      return r * st.size + c;
    }
    ...
    return Container(
      ...
      child: GestureDetector(
        onPanStart: (d) {
          ...
          // start fresh if grabbing the start; else continue from head
          if (i == st.level.start) {
            st.beginAt(i);
          } else {
            st.extendTo(i);
          ...
          decoration: BoxDecoration(
            gradient: primary
                ? const LinearGradient(
                    colors: [Color(0xFFC9701F), Color(0xFFFF9D3D)])
                : null,
